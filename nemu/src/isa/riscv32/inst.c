...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, int32_t signed_src1 = (int32_t)src1; uint32_t shamt = BITS(s->isa.inst, 24, 20); if (BITS(shamt, 5, 5) == 0) R(rd) = signed_src1 >> shamt);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, uint32_t usigned_src1 = (uint32_t)src1; uint32_t shamt = BITS(s->isa.inst, 24, 20); if (BITS(shamt, 5, 5) == 0) R(rd) = usigned_src1 >> shamt);
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, uint32_t usigned_src1 = (uint32_t)src1; uint32_t shamt = BITS(s->isa.inst, 24, 20); if (BITS(shamt, 5, 5) == 0) R(rd) = usigned_src1 << shamt);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & ~1;
                                                                    ...
                                                                    else if(rd == 0 && src1 == R(1)){  //ret = jalr x0, 0(x1),确保rs1 = x1
                                                                      #ifdef CONFIG_FTRACE
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lu     , I, R(rd) = SEXT(Mr(src1 + imm, 2), 16));
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4), 32));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, int csr_addr = (int)imm;      // CSR地址存储在immediate字段中
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, int32_t signed_src1 = (int32_t)src1; R(rd) = signed_src1 >> (src2 & 0x1f));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (src2 == 0) ? -1 : 
                                                               ((int32_t)src1 == INT32_MIN && (int32_t)src2 == -1) ? INT32_MIN : 
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (src2 == 0) ? src1 : 
                                                               ((int32_t)src1 == INT32_MIN && (int32_t)src2 == -1) ? 0 : 
                                                               (int32_t)src1 % (int32_t)src2);
