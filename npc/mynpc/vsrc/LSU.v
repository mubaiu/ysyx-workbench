module LSU(
    ...
    assign io_lsu_araddr = not_align ? addr_reg : {addr_reg[31:2], 2'b00};
    assign io_lsu_awaddr = not_align ? addr_reg : {addr_reg[31:2], 2'b00};
    assign io_lsu_wdata  = store_data_reg; 
    assign lsu_ready     = lsu_ready_reg;
    assign mem_to_reg    = (io_lsu_rvalid && load_flag) ? 1'b1 : 1'b0;

    assign io_lsu_wstrb  = io_lsu_wstrb_reg;

    reg [31:0] addr_reg        ;
    reg [31:0] store_data_reg  ;
    reg        lsu_ready_reg   ;
    reg        load_flag       ;
    reg [3:0]  io_lsu_wstrb_reg;
    ...
    always @(*) begin
        if (mem_read) begin
            ...
            case (lsu_wmask) 
                4'b0001: begin //SB
                    case (addr[1:0])
                        2'b00: io_lsu_wstrb_reg = 4'b0001;  // 字节0
                        2'b01: io_lsu_wstrb_reg = 4'b0010;  // 字节1
                        2'b10: io_lsu_wstrb_reg = 4'b0100;  // 字节2
                        2'b11: io_lsu_wstrb_reg = 4'b1000;  // 字节3
                    endcase
                end
                4'b0011: begin //SH
                    case (addr[1]) 
                        1'b0: io_lsu_wstrb_reg = 4'b0011;  // 半字0-1
                        1'b1: io_lsu_wstrb_reg = 4'b1100;  // 半字2-3
                    endcase
                end
                4'b1111: io_lsu_wstrb_reg = 4'b1111; //SW
                default: io_lsu_wstrb_reg = 4'b0000; // 默认按
            endcase
