...
static void welcome() {
  ...
  Log("Wtrace: %s", MUXDEF(ENABLE_WAVE_TRACE, ANSI_FMT("ON", ANSI_FG_GREEN), ANSI_FMT("OFF", ANSI_FG_RED)));\
...
{
  if (elf_file == NULL) {
    elf_file = "/home/mubai/ysyx-workbench/am-kernels/tests/cpu-tests/build/dummy-riscv32e-npc.elf";
    printf("Manually setting ELF file to: %s\n", elf_file);
  }
  ...
  for(int i = 0; i < ehdr.e_shnum; i++)
  {
    ...
    if(shdr.sh_type == SHT_SYMTAB)
    {
      ...
      symbol = (Symbol *)malloc(sym_count * sizeof(Symbol));
...
// export "C" void display_callfunc_dpi(int pc, int func_addr) {
//   display_callfunc((word_t)pc, (word_t)func_addr);
// }
...
// export "C" void display_retfunc_dpi(int pc) {
//   display_retfunc((word_t)pc);
// }
...
/*instruction trace*/
  ...
  IFDEF(CONFIG_DIFFTEST, (diff_so_file, img_size, difftest_port));
